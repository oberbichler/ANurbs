...
public:     // static methods
    static Pointer<CurveGeometry> insert_knots(const CurveGeometry& geometry, std::vector<double> knots)
    {
        ...
        Pointer<CurveGeometry> refined = new_<CurveGeometry>(degree, nb_poles_refined, true); // FIXME: check is_rational
        ...
        while (j >= 0) {
            while (knots[j] <= geometry.knot(i - 1) && i > a + 1) {
                ...
                refined->knot(k - 1) = geometry.knot(i - 1);
            ...
            for (Index l = 1; l < degree + 1; l++) {
                ...
                auto alpha = refined->knot(k + l - 1) - knots[j];
