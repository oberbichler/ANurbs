...
TEST_CASE( "Throws exception for unsupported degree", "[IntegrationPoints]" ) {
    ...
    CHECK_THROWS( IntegrationPoints<double>::compute(  0, domain) );
    CHECK_THROWS( IntegrationPoints<double>::compute(100, domain) );
}
...
TEST_CASE( "Integration points of domain [0, 1]", "[IntegrationPoints]" ) {
    ...
    auto integrationPoints = IntegrationPoints<double>::compute(degree, domain);
...
TEST_CASE( "Integration points of domain [[0, 1], [0, 1]]",
    ...
    auto integrationPoints = IntegrationPoints<double>::compute(degreeU,
